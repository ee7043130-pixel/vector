...
//角度
  function updateAngleDisplays() {
    ...
    const radius = 0.7;
    const pairs = [
      {v1: vecA, v2: vecB, arc: angleArcs[0], label: angleLabels[0], radius: radius, color: 0xffff00, name: '∠ AB', visible: isAVisible && isBVisible},
      {v1: vecA, v2: vecC, arc: angleArcs[1], label: angleLabels[1], radius: radius * 0.85, color: 0x00ffff, name: '∠ AC', visible: isAVisible && isCVisible},
      {v1: vecB, v2: vecC, arc: angleArcs[2], label: angleLabels[2], radius: radius * 0.7, color: 0xff00ff, name: '∠ BC', visible: isBVisible && isCVisible}
    ];
    ...
    pairs.forEach(({v1, v2, arc, label, radius, color, name, visible}) => {
      ...
      if(visible && v1.length() > 0.1 && v2.length() > 0.1 && angle > 1){
        ...
        updateAngleSprite(label, `${name}: ${angle.toFixed(1)}°`, color);
        label.visible = true;
      } else {
        arc.visible = false;
        label.visible = false;
...
function updateDotProductGraph() {
  ...
  function updateUnitCircleGraph() {
    ...
    const cosText = `cosθ=${cosTheta.toFixed(2)}`;
    ...
    const sinText = `sinθ=${sinTheta.toFixed(2)}`;
    ...
    const thetaText = `θ=${(angle * 180 / Math.PI).toFixed(1)}°`;
